...
class GenomicDeletionAmbiguous(Translator):
    ...
    async def translate(
        ...
        classification: GenomicDeletionAmbiguousClassification = validation_result.classification  # noqa: E501
        vrs_variation = None
        outer_coords = None
        ...
        if endpoint_name == Endpoint.NORMALIZE:
            vrs_variation = self.hgvs_dup_del_mode.interpret_variation(
                AltType.DELETION_AMBIGUOUS, seq_loc, warnings, hgvs_dup_del_mode, ac,
            ...
            vrs_variation = self.hgvs_dup_del_mode.default_mode(
                AltType.DELETION_AMBIGUOUS, outer_coords, "del", seq_loc, ac,
                baseline_copies=baseline_copies, copy_change=copy_change
            )
